...
def main():
    ...
    model3D.load_state_dict(torch.load('./checkpoint/sch2_phase1_avec_full_weight_' + country_name + '.ckpt'), strict=False)
    ...
    for epoch in range(args.epochs):
        ...
        if variable_acc > best_acc:
            ...
            torch.save({
                ...
                './checkpoint/' + 'sch3_avec_full_weight_' + country_name  + '.ckpt')

        # save the losses avg in .csv file
        with open("loss_avg_sch3_avec_full_weight_" + country_name + ".csv", 'a') as file:
