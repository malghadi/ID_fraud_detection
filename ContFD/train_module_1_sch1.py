...
from model_classify import SiameseNetwork as Model3D
   ...
   # Create model    
    ...
    for epoch in range(args.epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            # print(torch.sum(torch.max(y1o_softmax, 1)[1] == label_image_1))
            # get_corrects += torch.sum(torch.max(y1o_softmax, 1)[1] == label_image_1)
            # get_corrects += torch.sum(torch.max(y2o_softmax, 1)[1] == label_image_2)

            loss_total = loss_cross_entropy_1 + loss_cross_entropy_2 + loss_contrastive_densNet_latent +  loss_image_total_1 + loss_image_total_2
        ...
        with open("loss_avg_module_1_sch1_" + country_name + ".csv", 'a') as file:
...
def _save_best_model(model, best_acc, epoch, country_name):
    ...
    torch.save(state,
               './checkpoint/' + 'sch1_' + country_name + '.ckpt')
